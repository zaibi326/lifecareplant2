      ...
      <header className="flex items-end justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Daily filling output. Bulk gas auto-deducted per cylinder capacity.
          </p>
        </div>
        ...
        {(data ?? []).map((p: any) => (
          <Card key={p.id} className="p-4 flex items-center gap-3">
            <div
              ...
              style={{ background: p.gas_types?.color || "var(--warning)" }}
            >
...
function ProductionForm({ onDone }: { onDone: () => void }) {
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      toast.success(
        capacity != null ? "Production logged — bulk gas deducted" : "Production logged",
      );
        ...
        <div className="rounded-lg border p-3 bg-muted/30 text-xs">
          {capacity != null ? (
            <>
              Capacity{" "}
              <b>
                {capacity} {selectedSize?.capacity_unit ?? "m3"}
              </b>{" "}
              × {qty || 0} = gas consumed <b>{formatM3(consumed)}</b> (auto-deducted from bulk).
            </>
          ) : (
            <>
              No capacity set for this size. Set it in <b>Settings → Sizes</b> to auto-deduct bulk
              gas.
            </>
          )}
        </div>
