...
function ReportsPage() {
  ...
  const totals = useMemo(() => {
    ...
    const delivered = ms.filter((m) => m.type === "deliver").reduce((a, b) => a + Number(b.quantity ?? 0), 0);
  ...
  const dailySeries = useMemo(() => {
    ...
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
      ...
      map.set(key, { day: new Date(key).toLocaleDateString("en-GB", { day: "2-digit", month: "short" }), Received: 0, Delivered: 0 });
    }
    ms.forEach((m) => {
    ...
    <div className="space-y-6">
      <header className="flex items-end justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{formatDate(from)} → {formatDate(to)}</p>
        </div>
      ...
      <Card className="p-4 grid grid-cols-2 md:grid-cols-3 gap-3 items-end">
        ...
        <div className="grid grid-cols-3 gap-2 md:col-span-1 col-span-2">
          {[
            ...
            <Button key={lbl as string} variant="secondary" className="h-11" onClick={() => {
              const t = new Date();
              setTo(t.toISOString().slice(0, 10));
              setFrom(new Date(Date.now() - (days as number) * 86400000).toISOString().slice(0, 10));
            }}>{lbl}</Button>
          ))}
        </div>
      </Card>
      ...
      <section className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <Kpi icon={ArrowDownToLine} label="Received" value={totals.received.toLocaleString()} tone="brand" />
        <Kpi icon={ArrowUpFromLine} label="Delivered" value={totals.delivered.toLocaleString()} tone="default" />
        <Kpi icon={Factory} label="Produced" value={totals.produced.toLocaleString()} tone="success" />
      ...
      <Card className="p-4">
        ...
        {!isLoading && topCustomers.length === 0 && <div className="text-sm text-muted-foreground">No data in this range.</div>}
        <div className="divide-y">
          {topCustomers.map((c, i) => (
            <div key={i} className="py-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <div className="size-7 rounded-full bg-brand/10 text-brand grid place-items-center text-xs font-bold">{i + 1}</div>
                <div className="min-w-0">
                  <div className="font-semibold truncate">{c.name}</div>
                  <div className="text-xs text-muted-foreground">{c.qty} cyl • Paid {formatCurrency(c.paid)}</div>
                </div>
              </div>
...
function Kpi({ icon: Icon, label, value, tone }: any) {
  const cls = tone === "brand" ? "bg-brand/10 text-brand" : tone === "success" ? "bg-success/15 text-success" : tone === "warn" ? "bg-warning/15 text-warning" : "bg-muted text-foreground";
  return (
    <Card className="p-3">
