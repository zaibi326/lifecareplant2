    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Company, gases, sizes{isAdmin ? " and staff" : ""}.
        </p>
      </header>
      ...
      <Tabs defaultValue="company" className="w-full">
        <TabsList className={`grid ${isAdmin ? "grid-cols-6" : "grid-cols-5"} w-full h-11`}>
          ...
          <TabsTrigger value="gases" className="gap-2">
            <Flame className="size-4" /> Gases
          </TabsTrigger>
          <TabsTrigger value="sizes" className="gap-2">
            <Package className="size-4" /> Sizes
          </TabsTrigger>
          <TabsTrigger value="parts" className="gap-2">
            <Wrench className="size-4" /> Parts
          </TabsTrigger>
    ...
    <Card className="p-5 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Sheet open={open} onOpenChange={setOpen}>
          ...
          <SheetContent side="right" className="w-full sm:max-w-md overflow-y-auto">
            <SheetHeader>
              <SheetTitle>Add Staff / Admin</SheetTitle>
            </SheetHeader>
            <form onSubmit={onSubmit} className="mt-6 space-y-4">
              <div>
                <Label className="text-xs">Full Name*</Label>
                <Input name="full_name" required className="mt-1.5 h-11" />
              </div>
              <div>
                <Label className="text-xs">Email*</Label>
                <Input name="email" type="email" required className="mt-1.5 h-11" />
              </div>
              <div>
              ...
              <p className="text-[11px] text-muted-foreground">
                Share the email and temporary password with the user. They can change it later.
              </p>
            </form>
          </SheetContent>
        </Sheet>
      ...
      <div className="divide-y">
        ...
        {!isLoading && (data ?? []).length === 0 && (
          <div className="py-4 text-sm text-muted-foreground">No staff yet.</div>
        )}
        {(data ?? []).map((u: any) => (
            ...
            >
              ...
              <AlertDialogContent>
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction
                    onClick={() => pwdMut.mutate({ user_id: u.id, password: newPwd })}
                  >
                    Update
                  </AlertDialogAction>
                </AlertDialogFooter>
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This will permanently delete {u.email} and revoke access.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>Cancel</AlertDialogCancel>
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <Field
            ...
            name="invoice_prefix"
            defaultValue={data?.invoice_prefix ?? "INV-"}
          />
          <Field
            label="Tax %"
            name="tax_percent"
            type="number"
            defaultValue={String(data?.tax_percent ?? 0)}
          />
        </div>
        <div>
          ...
          <Textarea
            name="invoice_footer"
            defaultValue={data?.invoice_footer ?? ""}
            rows={2}
            className="mt-1.5"
          />
        </div>
        ...
        <div className="rounded-lg border p-4 bg-muted/20 space-y-1.5">
          ...
          />
          <p className="text-[11px] text-muted-foreground">
            Total cylinders your plant owns. Used on the Stock page to reconcile Owned = In Plant +
            With Customers. The difference should always be zero.
          </p>
        </div>
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase
        .from("gas_types")
        .insert({ name: name.trim(), color, active: true });
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  return (
    <Card className="p-5 space-y-4">
      <div className="flex gap-2">
        <Input
          placeholder="Gas name (e.g. Oxygen)"
          value={name}
        ...
        />
        <Button onClick={() => create.mutate()} disabled={create.isPending} className="h-11 gap-2">
      ...
      <div className="divide-y">
          ...
          <div key={g.id} className="py-3 flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <Badge variant={g.active ? "secondary" : "outline"} className="text-[10px] mt-0.5">
                  {g.active ? "Active" : "Inactive"}
                </Badge>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Button variant="ghost" size="sm" onClick={() => toggle.mutate(g)}>
        ...
        {(data ?? []).length === 0 && (
          <div className="py-4 text-sm text-muted-foreground">No gas types yet.</div>
        )}
      </div>
    </Card>
  );
...
function SizesPanel() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("cylinder_sizes").select("*").order("name")).data ?? [],
  });
  ...
  const updateCapacity = useMutation({
    mutationFn: async (v: { id: string; capacity: number | null; capacity_unit: string }) => {
      const { error } = await supabase
        .from("cylinder_sizes")
        .update({ capacity: v.capacity, capacity_unit: v.capacity_unit })
        .eq("id", v.id);
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    <Card className="p-5 space-y-4">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Capacity is the gas per cylinder (e.g. 9.90 m³). Used to auto-deduct bulk gas on
          production.
        </p>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-[1fr_100px_100px_110px_auto] gap-2">
        <Input
          placeholder="Name (e.g. 9.90)"
          value={name}
          onChange={(e) => setName(e.target.value)}
          className="h-11"
        />
      ...
      <div className="divide-y">
          ...
          <div key={s.id} className="py-3 flex items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="text-xs text-muted-foreground">
                {s.capacity != null
                  ? `Capacity ${s.capacity} ${s.capacity_unit ?? "m3"}`
                  : "No capacity set"}
            ...
            <div className="flex items-center gap-1 shrink-0">
              <Input
                ...
                onBlur={(e) => {
                  ...
                  if (val !== (s.capacity ?? null))
                    updateCapacity.mutate({
                      id: s.id,
                      capacity: val,
                      capacity_unit: s.capacity_unit ?? "m3",
                    });
                }}
              />
              <Button
        ...
        {(data ?? []).length === 0 && (
          <div className="py-4 text-sm text-muted-foreground">No sizes yet.</div>
        )}
      </div>
    </Card>
  );
...
function PartSizesPanel() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("part_sizes").select("*").order("sort_order").order("label")).data ?? [],
  });

  const partsStock = useQuery({
    queryKey: ["parts_stock_for_settings"],
    queryFn: async () =>
      (await supabase.from("parts_stock").select("kind,size,quantity")).data ?? [],
  });
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const sort_order =
        (data ?? []).reduce((m: number, r: any) => Math.max(m, Number(r.sort_order ?? 0)), 0) + 10;
      const { error } = await supabase
        .from("part_sizes")
        .insert({ label: label.trim(), sort_order, active: true });
      if (error) throw error;
    },
    onSuccess: () => {
      toast.success("Size added");
      setLabel("");
      qc.invalidateQueries({ queryKey: ["part_sizes_all"] });
      qc.invalidateQueries({ queryKey: ["part_sizes"] });
    },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  return (
    <Card className="p-5 space-y-4">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Yahan jo sizes add karein gy, wo deliver entry aur Stock tab dono mein dropdown mein aein
          gy.
        </p>
      </div>
      ...
      <div className="divide-y">
          ...
          return (
            <div key={s.id} className="py-3 flex items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground">
                  Valve stock: {st.valve} • Spindle stock: {st.spindle}
                </div>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <Button
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        Iss size ka parts stock bhi affect ho sakta hai. Disable karna safer hai.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel>Cancel</AlertDialogCancel>
        ...
        {(data ?? []).length === 0 && (
          <div className="py-4 text-sm text-muted-foreground">No part sizes yet.</div>
        )}
      </div>
    </Card>
  );
...
function PermissionsMatrix() {
  const rows: { module: string; actions: { name: string; admin: boolean; staff: boolean }[] }[] = [
    { module: "Dashboard", actions: [{ name: "View KPIs & charts", admin: true, staff: true }] },
    {
      module: "Customers",
      actions: [
    ...
    {
      module: "Movements (Receive / Deliver)",
      actions: [
    ...
    {
      module: "Stock",
      actions: [{ name: "View plant & customer stock", admin: true, staff: true }],
    },
    {
      module: "Reports",
      actions: [
    ...
    {
      module: "Settings — Staff Management",
      actions: [
    ...
    <Card className="p-5 space-y-4">
      <div>
        <h3 className="font-display font-bold flex items-center gap-2">
          <ShieldCheck className="size-4 text-brand" /> Role Permissions
        </h3>
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        <Badge className="bg-brand text-brand-foreground gap-1">
          <ShieldCheck className="size-3" /> Admin — full access
        </Badge>
        <Badge variant="secondary" className="gap-1">
          <Users className="size-3" /> Staff — daily operations
        </Badge>
      </div>
            ...
            {rows.map((r) => (
              <Fragment key={r.module}>
                <tr className="bg-muted/40">
                  <td
                    colSpan={3}
                    className="py-2 px-5 font-display font-bold text-xs uppercase tracking-wider"
                  >
                    {r.module}
                  </td>
                </tr>
                {r.actions.map((a, i) => (
                  <tr key={r.module + i} className="border-b last:border-0">
                    ...
                    <td className="py-2.5 px-3 text-center">
                      {a.admin ? (
                        <span className="inline-flex size-6 rounded-full bg-success/15 text-success items-center justify-center">
                          ✓
                        </span>
                      ) : (
                        <span className="inline-flex size-6 rounded-full bg-muted text-muted-foreground items-center justify-center">
                          –
                        </span>
                      )}
                    </td>
                    <td className="py-2.5 px-5 text-center">
                      {a.staff ? (
                        ...
                        <span className="inline-flex size-6 rounded-full bg-destructive/10 text-destructive items-center justify-center">
                          ✕
                        </span>
                      )}
                    </td>
      ...
      <p className="text-[11px] text-muted-foreground">
        Note: Roles are enforced by Lovable Cloud security policies. To change a user's role, go to
        the <strong>Staff</strong> tab (admin only).
      </p>
    </Card>
  );
}
