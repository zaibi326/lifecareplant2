    ...
    <div className="space-y-5">
      <header className="flex items-end justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Bulk gas received from suppliers. KG auto-converts to m³.</p>
        </div>
        <Sheet open={open} onOpenChange={setOpen}>
          <SheetTrigger asChild>
            <Button className="gap-2"><Plus className="size-4" /> New Purchase</Button>
          </SheetTrigger>
          <SheetContent side="right" className="w-full sm:max-w-lg overflow-y-auto">
            <SheetHeader><SheetTitle>Record Gas Purchase</SheetTitle></SheetHeader>
      ...
      <Card className="p-4 flex items-center justify-between">
        <div>
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Purchased (filtered)</div>
      ...
      <div className="relative">
        ...
        <Input placeholder="Search supplier / gas / invoice" value={q} onChange={(e) => setQ(e.target.value)} className="pl-9 h-11" />
      </div>

      <div className="space-y-2">
        {isLoading && <Card className="p-6 text-sm text-muted-foreground">Loading…</Card>}
        {!isLoading && filtered.length === 0 && <Card className="p-8 text-center text-sm text-muted-foreground">No purchases yet.</Card>}
        {(filtered as any[]).map((p) => (
          <Card key={p.id} className="p-4 flex items-center gap-3">
            <div className="size-10 rounded-lg grid place-items-center text-white font-bold text-xs" style={{ background: p.gas_types?.color || "var(--brand)" }}>
              {(p.gas_types?.name ?? "—").slice(0, 2).toUpperCase()}
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-semibold truncate">{p.gas_types?.name ?? "—"} • {p.suppliers?.name ?? "No supplier"}</div>
              <div className="text-xs text-muted-foreground truncate">
                {formatDate(p.date)} • {formatM3(p.cubic_meter)}
                {p.unit === "kg" ? ` (${Number(p.kg ?? p.quantity).toLocaleString()} kg × ${p.conversion_factor ?? DEFAULT_OXYGEN_FACTOR})` : ""}
            ...
            <div className="text-right flex flex-col items-end gap-1">
              ...
              <Badge variant="secondary" className="text-[10px]">{formatCurrency(p.total_amount)}</Badge>
            </div>
            <DropdownMenu>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(v) => !v && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will reduce bulk gas inventory for this gas type.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
    ...
    <form onSubmit={(e) => { e.preventDefault(); save.mutate(new FormData(e.currentTarget)); }} className="mt-6 space-y-4">
      <div>
        ...
        <Select value={supplier} onValueChange={setSupplier}>
          <SelectTrigger className="mt-1.5 h-11"><SelectValue placeholder="Select supplier" /></SelectTrigger>
          <SelectContent>{(lookups?.suppliers ?? []).map((s: any) => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}</SelectContent>
        </Select>
      </div>
      <div>
        <Label className="text-xs">Gas Type*</Label>
        <Select value={gas} onValueChange={setGas}>
          <SelectTrigger className="mt-1.5 h-11"><SelectValue placeholder="Select gas" /></SelectTrigger>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input type="number" min={0} step="0.01" value={quantity || ""} onChange={(e) => setQuantity(Number(e.target.value))} className="mt-1.5 h-11" />
        </div>
        <div>
      ...
      {unit === "kg" && (
        <div className="rounded-lg border p-3 bg-muted/30 text-xs">
          {isOxygenKg
            ...
            : <>KG entered. Stored as <b>{formatM3(cubicMeter)}</b> using factor <b>{factor}</b>. (Configure factor in Settings.)</>}
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input type="number" min={0} step="0.01" value={rate || ""} onChange={(e) => setRate(Number(e.target.value))} className="mt-1.5 h-11" />
        </div>
        <div>
