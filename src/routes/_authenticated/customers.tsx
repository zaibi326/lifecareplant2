...
function CustomersPage() {
    ...
    queryFn: async () => {
      const [
        ...
        supabase
          ...
          .select("customer_id,type,quantity,total_amount,cylinder_size_id,gas_type_id"),
        supabase.from("payments").select("customer_id,amount"),
        supabase
          .from("customer_opening_balances")
          .select("customer_id,quantity,condition,cylinder_size_id,gas_type_id"),
      ...
      const comboMap = new Map<
        string,
        { gas: string; size: string; opening: number; received: number; delivered: number }
      >();
      const ensure = (gid: string, sid: string) => {
        ...
        if (!v) {
          v = {
            gas: gasNames.get(gid) ?? "—",
            size: sizeNames.get(sid) ?? "—",
            opening: 0,
            received: 0,
            delivered: 0,
          };
      ...
      const sizeBreakdown = Array.from(comboMap.values())
        .map((v) => ({
          name: `${v.gas} • ${v.size}`,
          ...v,
          qty: v.opening + v.received - v.delivered,
        }))
  ...
  const addRow = () =>
    setOpenRows((r) => [
      ...r,
      {
        gas_type_id: refs?.gases[0]?.id ?? "",
        cylinder_size_id: refs?.sizes[0]?.id ?? "",
        condition: "filled",
        quantity: 1,
      },
    ]);
  const updRow = (i: number, patch: Partial<OpenRow>) =>
    setOpenRows((r) => r.map((x, idx) => (idx === i ? { ...x, ...patch } : x)));
    ...
    <div className="space-y-5">
          ...
          <form onSubmit={onSubmit} className="mt-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field
                label="Category"
                name="category"
                placeholder="Industrial / Medical"
            ...
            <div className="rounded-lg border p-3 space-y-2">
              ...
              {openRows.length === 0 && (
                <p className="text-xs text-muted-foreground">
                  Customer ke pas pehle se mojood cylinders gas type + size + condition ke hisab se
                  add karein.
                </p>
              )}
              {openRows.map((r, i) => (
                <div
                  key={i}
                  className="grid grid-cols-[1fr_1fr_90px_70px_auto] gap-1.5 items-center"
                >
                  <Select
                    value={r.gas_type_id}
                    onValueChange={(v) => updRow(i, { gas_type_id: v })}
                  >
                    <SelectTrigger className="h-9 text-xs">
                      <SelectValue placeholder="Gas" />
                    </SelectTrigger>
                    <SelectContent>
                      {(refs?.gases ?? []).map((g: any) => (
                  ...
                  <Select
                    ...
                    onValueChange={(v) => updRow(i, { cylinder_size_id: v })}
                  >
                    <SelectTrigger className="h-9 text-xs">
                      <SelectValue placeholder="Size" />
                    </SelectTrigger>
                    <SelectContent>
                      {(refs?.sizes ?? []).map((s: any) => (
                  ...
                  <Select
                    value={r.condition}
                    onValueChange={(v: any) => updRow(i, { condition: v })}
                  >
                    <SelectTrigger className="h-9 text-xs">
                      <SelectValue />
                    </SelectTrigger>
                  ...
                  <Input
                    ...
                    min={0}
                    value={r.quantity}
                    onChange={(e) => updRow(i, { quantity: Number(e.target.value) })}
      ...
      <Dialog open={breakdownOpen} onOpenChange={setBreakdownOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Cylinders by Size</DialogTitle>
          </DialogHeader>
            ...
            >
              ...
              <div className="flex items-stretch gap-2 shrink-0">
                <div className="text-right rounded-lg bg-brand/5 px-2.5 py-1.5 min-w-[64px]">
                  ...
                  <div
                    className={`font-display font-bold text-base ${c.balance.out > 0 ? "text-brand" : "text-muted-foreground"}`}
                  >
                    {c.balance.out}
                  </div>
                </div>
                <div className="text-right rounded-lg bg-warning/5 px-2.5 py-1.5 min-w-[80px]">
                  ...
                  <div
                    className={`font-display font-bold text-base ${c.balance.due > 0 ? "text-destructive" : "text-success"}`}
                  >
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(v) => !v && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Ye action permanent hai. Customer ke saath uski tamam movements, payments, aur opening
              balances bhi delete ho jaein gy.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
