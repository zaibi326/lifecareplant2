...
import { ArrowDownToLine, ArrowUpFromLine, Wallet, Factory, Package, Users, TrendingUp, AlertCircle, Fuel, Sparkles, CheckCircle2, AlertTriangle, Info } from "lucide-react";
...
function Dashboard() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [movements, payments, production, customers, gases, allMoves, allPays, purchases, allProduction, monthMoves, monthExp, settings] = await Promise.all([
        ...
        supabase.from("cylinder_movements").select("total_amount,type,date").eq("type", "deliver").gte("date", monthStart),
      ...
      return {
        ...
        monthRevenue: (monthMoves.data ?? []).reduce((a: number, m: any) => a + Number(m.total_amount ?? 0), 0),
        monthExpenses: (monthExp.data ?? []).reduce((a: number, e: any) => a + Number(e.amount ?? 0), 0),
  ...
  const bulkRows = Array.from(bulkBalances.entries())
    .map(([id, v]) => ({ id, name: gasNameById.get(id)?.name ?? "Gas", color: gasNameById.get(id)?.color ?? null, ...v }))
    .sort((a, b) => b.remaining - a.remaining);


  // Owner insights (rule-based)
  const topDebtorMap = new Map<string, { name: string; due: number }>();
  for (const c of (data?.customers ?? [])) topDebtorMap.set(c.id, { name: c.name ?? "Customer", due: Number(c.opening_due ?? 0) });
  for (const m of all) {
  ...
  const topDebtors = Array.from(topDebtorMap.values()).filter((d) => d.due > 0).sort((a, b) => b.due - a.due).slice(0, 3);
  ...
  const insights = generateInsights({
    ...
    bulkLow: bulkRows.filter((b) => b.remaining <= 0).map((b) => ({ name: b.name, remaining: b.remaining })),
    topDebtors,
  });
  ...
  return (
    <div className="space-y-6">
        ...
        <section className="bg-card border rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Auto-generated from your latest figures</p>
            </div>
          </div>
      ...
      <section className="bg-card border rounded-2xl p-5">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h2 className="font-display font-bold flex items-center gap-2"><Fuel className="size-4 text-brand" /> Bulk Gas Inventory</h2>
            <p className="text-xs text-muted-foreground">Purchased minus consumed in filling (in m³)</p>
          </div>
          <Link to="/purchases" className="text-xs text-brand font-medium">Purchases</Link>
        </div>
        {bulkRows.length === 0 ? (
            ...
            {bulkRows.map((b) => (
              <div key={b.id} className="rounded-xl border p-3">
                <div className="flex items-center gap-2">
                  <span className="size-3 rounded-full" style={{ background: b.color || "var(--color-brand)" }} />
                  <span className="text-sm font-semibold truncate">{b.name}</span>
                </div>
                <div className={`font-display font-bold text-lg mt-1 ${b.remaining < 0 ? "text-destructive" : ""}`}>{formatM3(b.remaining)}</div>
                <div className="text-[10px] text-muted-foreground mt-0.5">In {formatM3(b.purchased)} • Used {formatM3(b.consumed)}</div>
              </div>
            ))}
          </div>
        )}
...
function InsightCard({ insight }: { insight: Insight }) {
  const map = {
    critical: { icon: AlertCircle, cls: "border-destructive/40 bg-destructive/5", icon_cls: "text-destructive" },
    warning: { icon: AlertTriangle, cls: "border-warning/40 bg-warning/5", icon_cls: "text-warning" },
    positive: { icon: CheckCircle2, cls: "border-success/40 bg-success/5", icon_cls: "text-success" },
