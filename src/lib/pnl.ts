...
export type PnlInput = {
  range: DateRange;
  deliveries: { date: string; total_amount: number | null; rate: number | null; quantity: number | null }[];
...
export function computePnl(i: PnlInput): PnlResult {
  ...
  if (deliveryExpense > 0) byCategory["Vehicle / Delivery"] = (byCategory["Vehicle / Delivery"] ?? 0) + deliveryExpense;
...
export type RentalConfig = { enabled: boolean; period: "daily" | "weekly" | "monthly"; rate: number };

export function periodsBetween(fromISO: string, toISO: string, period: RentalConfig["period"]): number {
...
export function answerOwnerQuery(qRaw: string, ctx: AssistantContext): AssistantAnswer {
  ...
  if ((q.includes("remaining") || q.includes("left") || q.includes("stock") || q.includes("available")) && gasMatch && !/\d/.test(q)) {
    return { answer: `${gasMatch.name}: ${gasMatch.remaining.toFixed(2)} m³ remaining in bulk.`, matched: true };
  }
  ...
  if (q.includes("cylinder") || /\d/.test(q)) {
    const cand = ctx.filledByGasSize.filter((r) => {
      ...
      const gasHit = ctx.bulkByGas.some((b) => q.includes(b.name.toLowerCase())) ? q.includes(g) : true;
    ...
    if (cand.length > 0) {
      ...
      return { answer: `${total} filled ${label} cylinder${total === 1 ? "" : "s"} available.`, matched: true };
    }
  }

  // Profit
  if (q.includes("profit")) {
    if (q.includes("today")) return { answer: `Today's net profit: ${currency(ctx.todayProfit)}.`, matched: true };
    if (q.includes("month")) return { answer: `This month's net profit: ${currency(ctx.monthProfit)}.`, matched: true };
    return { answer: `Today ${currency(ctx.todayProfit)} · This month ${currency(ctx.monthProfit)}.`, matched: true };
  }

  // Debtors
  if (q.includes("owe") || q.includes("debtor") || q.includes("outstanding") || q.includes("due")) {
    if (ctx.topDebtor) return { answer: `${ctx.topDebtor.name} owes the most: ${currency(ctx.topDebtor.due)}. Total outstanding ${currency(ctx.outstanding)}.`, matched: true };
    return { answer: `Total outstanding is ${currency(ctx.outstanding)}.`, matched: true };
  }

  // Vehicle expense
  if (q.includes("vehicle") && (q.includes("expense") || q.includes("cost") || q.includes("highest"))) {
    if (ctx.topVehicleExpense) return { answer: `${ctx.topVehicleExpense.name} generated the highest delivery expense: ${currency(ctx.topVehicleExpense.total)}.`, matched: true };
    return { answer: "No vehicle expenses recorded yet.", matched: true };
  }

  // Best selling gas
  if (q.includes("best") || q.includes("most sold") || q.includes("selling")) {
    if (ctx.bestSellingGas) return { answer: `Best selling gas: ${ctx.bestSellingGas.name} (${ctx.bestSellingGas.qty} cylinders delivered).`, matched: true };
  }

  return { answer: "I couldn't match that. Try: \"How much oxygen is remaining?\", \"What is today's profit?\", or \"Which customer owes the most?\"", matched: false };
}
